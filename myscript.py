...
from custom_config import API_CODES, API_LINKS
import requests
...
try:
    ...
    agent.checkpoint_send(
        checkpoint_url=e.checkpoint_url,
        forward_url=ch_handle["navigation"]["forward"],
        choice=min([x["value"] for x in e.types]),
        settings=settings,
    )
    while is_fail:
        try:
            api_req = requests.get(API_CODES)
